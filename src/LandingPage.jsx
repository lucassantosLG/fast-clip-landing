...
const InlineForm = ({ darkBg = false }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setStatus('success');
      setName(''); setPhone(''); setPhoneDisplay(''); setEmail('');
    } catch (error) {
  ...
  const btnClass = darkBg
    ? "font-karla w-full px-6 py-3 text-base bg-black-primary text-white font-bold rounded-lg hover:bg-gray-dark active:scale-95 transition-all duration-300 shadow-lg disabled:opacity-60 disabled:cursor-not-allowed"
    : "font-karla w-full px-6 py-3 text-base bg-bronze-light text-black-primary font-bold rounded-lg hover:bg-copper/80 active:scale-95 transition-all duration-300 shadow-lg disabled:opacity-60 disabled:cursor-not-allowed";
  ...
  return (
    <div className="mt-8 w-full">
      <form onSubmit={handleSubmit} className="max-w-3xl mx-auto flex flex-col sm:flex-row gap-2">
        <input type="text"  value={name}         onChange={(e) => setName(e.target.value)}  placeholder="Nome"          required disabled={status === 'sending'} className={inputClass} />
        <input type="tel"   value={phoneDisplay}  onChange={handlePhoneChange}               placeholder="(DD) WhatsApp" required maxLength={15} disabled={status === 'sending'} className={inputClass} />
        <input type="email" value={email}         onChange={(e) => setEmail(e.target.value)} placeholder="E-mail"        required disabled={status === 'sending'} className={inputClass} />
        <button type="submit" disabled={status === 'sending'} className={btnClass}>
          {status === 'sending' ? 'Enviando...' : 'Entrar na Lista'}
        </button>
      </form>
      {status === 'sending' && <p className={`text-center text-sm mt-3 font-karla animate-pulse ${feedbackColorSending}`}>Enviando...</p>}
      {status === 'success' && <p className={`text-center text-sm mt-3 font-karla font-bold ${feedbackColorSuccess}`}>✅ Cadastro realizado com sucesso! Em breve entraremos em contato.</p>}
      {status === 'error'   && <p className={`text-center text-sm mt-3 font-karla font-bold ${feedbackColorError}`}>❌ Erro ao cadastrar. Tente novamente.</p>}
    </div>
  );
};
...
const FastClipLandingPage = () => {
  ...
  const testimonials = [
    { name: "João Silva",   role: "Instrutor Tático",       text: "O Fast Clip mudou completamente minha rotina. Ajuste instantâneo e segurança incomparável.",              avatar: "https://ui-avatars.com/api/?name=João+Silva&background=8B6914&color=fff&size=128&bold=true&font-size=0.4"   },
    { name: "Maria Santos", role: "Agente de Segurança",    text: "Finalmente um cinto que acompanha minha agilidade. Praticidade e resistência em um só produto.",          avatar: "https://ui-avatars.com/api/?name=Maria+Santos&background=8B6914&color=fff&size=128&bold=true&font-size=0.4" },
    { name: "Pedro Costa",  role: "Profissional Outdoor",   text: "Uso em trilhas e expedições. A durabilidade e o ajuste rápido fazem toda a diferença.",                   avatar: "https://ui-avatars.com/api/?name=Pedro+Costa&background=8B6914&color=fff&size=128&bold=true&font-size=0.4"  }
  ];
  ...
  const bonuses = [
    { title: "25% de Desconto",    description: "Exclusivo para os primeiros cadastrados",  icon: <AiOutlinePercentage size={48} className="text-bronze" /> },
    { title: "Frete Grátis",       description: "Entrega gratuita em todo o Brasil",         icon: <MdLocalShipping     size={48} className="text-bronze" /> },
    { title: "Acesso Prioritário", description: "Receba antes do lançamento oficial",        icon: <RiVipCrownLine      size={48} className="text-bronze" /> }
  ];
    ...
    <div className="font-karla">
        ...
        <div className="container mx-auto px-6">
          ...
          <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
            <div className="text-center">
              <div className="flex justify-center mb-4"><IconColdre size={48} className="text-bronze" /></div>
              <h3 className="text-xl font-bold mb-3 text-black-primary uppercase tracking-tight">Funciona com o seu coldre</h3>
              <p className="text-gray-dark leading-relaxed">Funciona com os principais coldres disponíveis no mercado global.</p>
            </div>
            <div className="text-center">
              <div className="flex justify-center mb-4"><FiZap size={48} className="text-bronze" /></div>
              <h3 className="text-xl font-bold mb-3 text-black-primary uppercase tracking-tight">Clipagem em segundos</h3>
              <p className="text-gray-dark leading-relaxed">Você só precisa de 15 segundos. Simples. Rápido. Inovador.</p>
            </div>
            <div className="text-center">
              <div className="flex justify-center mb-4"><MdOutlineBeachAccess size={48} className="text-bronze" /></div>
              <h3 className="text-xl font-bold mb-3 text-black-primary uppercase tracking-tight">Conforto 24/7</h3>
              <p className="text-gray-dark leading-relaxed">Você escolhe a roupa. Nós garantimos que funcione. Não é apenas tático. É a sua solução para a folga.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 bg-gray-light">
        <div className="container mx-auto px-6">
          ...
          <div className="relative max-w-4xl mx-auto rounded-xl overflow-hidden shadow-2xl" style={{ backgroundColor: '#0a0a0a' }}>
            <div className="absolute inset-0 z-10 pointer-events-none" style={{ background: 'linear-gradient(to bottom, rgba(0,0,0,0.45) 0%, rgba(0,0,0,0.1) 40%, rgba(0,0,0,0.55) 100%)' }} />
            <img src="/images/cinto-coldre-nevoa.jpeg" alt="Fast Clip Solução" className="relative z-0 w-full object-cover opacity-80" />
          </div>
        ...
        <div className="container mx-auto px-6">
          ...
          <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {testimonials.map((testimonial, index) => (
              <div key={index} className="bg-white border border-gray-300 rounded-lg p-8 shadow-lg hover:shadow-xl transition-all duration-300">
                <div className="flex items-center mb-6">
                  <img src={testimonial.avatar} alt={testimonial.name} className="w-16 h-16 rounded-full object-cover mr-4 border-2 border-bronze" />
                  <div>
        ...
        <div className="container mx-auto px-6">
          ...
          <div className="grid md:grid-cols-3 gap-6 max-w-6xl mx-auto">
            {specifications.map((spec, index) => (
              <div key={index} className="bg-white border border-gray-300 rounded-lg p-6 hover:shadow-lg transition-all duration-300">
        ...
        <div className="container mx-auto px-6">
          ...
          <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
            {bonuses.map((bonus, index) => (
              <div key={index} className="bg-gray-light border-2 border-bronze rounded-lg p-8 text-center hover:scale-105 transition-all duration-300">
            ...
            {faqs.map((faq, index) => (
              <div key={index} className="bg-white border border-gray-300 rounded-lg overflow-hidden">
                ...
                >
                  ...
                  <span className="text-2xl text-bronze font-light">{faqOpen === index ? '−' : '+'}</span>
                </button>
                {faqOpen === index && (
      ...
      <footer className="bg-black-primary text-gray-medium py-12 border-t border-gray-dark">
        <div className="container mx-auto px-6 text-center">
          <div className="mb-6">
            <h3 className="font-display text-3xl text-white mb-2 tracking-wide">FAST CLIP</h3>
            <p className="text-gray-medium font-karla tracking-wider text-sm uppercase">Liberdade em Movimento</p>
          </div>
          <div className="border-t border-gray-dark pt-6">
            <p className="text-sm font-karla">© 2025 Fast Clip. Todos os direitos reservados.</p>
            <p className="text-sm mt-2 text-gray-medium font-karla">Desenvolvido para profissionais que não aceitam limites</p>
          </div>
        </div>
      </footer>
